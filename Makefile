...
build-os-arch: ensure_dir
	GOOS=$(OS) GOARCH=$(ARCH) go build -o $(OUTPUT_DIR)/$(BINARY)-$(OS)-$(ARCH) ./cmd/airflow-db-cleaner
...
release-os-arch: ensure_release_dir
	...
	@GOOS=$(OS) GOARCH=$(ARCH) go build -o $(RELEASE_DIR)/$(BINARY)-$(OS)-$(ARCH)/$(BINARY) ./cmd/airflow-db-cleaner
